...
import numpy as np
import h5py
from commonUtils import savePickle, loadPickle
...
class ShapeNetCore(Dataset):
    ...
    def get_statistics(self):
        ...
        if len(self.cate_synsetids) == len(cate_to_synsetid):
            # stats_save_path = os.path.join(stats_dir, 'stats_all_{0:s}.pt'.format(self.split))
            stats_save_path = os.path.join(stats_dir, 'stats_all.pt')
        else:
            # stats_save_path = os.path.join(stats_dir, 'stats_' + '_'.join(self.cate_synsetids) + '_{0:s}.pt'.format(self.split))
            stats_save_path = os.path.join(stats_dir, 'stats_' + '_'.join(self.cate_synsetids) + '.pt')
        if os.path.exists(stats_save_path):
            self.stats = loadPickle(stats_save_path)
            return self.stats
        ...
        savePickle(stats_save_path, self.stats)
        return self.stats
    ...
    def load(self):
        ...
        with h5py.File(self.path, mode='r', libver='latest') as f:
